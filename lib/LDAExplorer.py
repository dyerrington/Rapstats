...
class LDAExplorer:
    ...
    def __init__(self):
        ...
        try:
            ...
            directory = '/var/www/htdocs/rapalytics/data/albums/' # temp location
            albums  =   [f for f in os.listdir(directory) if os.path.isdir(directory + f)]
            files   =   [glob.glob(directory + file + '/*.txt') for file in albums]
            self.corpus_files   =   [item.split('/var/www/htdocs/rapalytics/data/albums/')[1] for files in files for item in files]
            corpus_files_meta   =   pd.DataFrame(self.corpus_files)
            corpus_files_meta.save(self.corpus_files_meta)
